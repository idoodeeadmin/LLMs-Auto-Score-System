...
async def generate_rubric_with_openai(question_text: str, total_score: float, tone="moderate", images=None):
    ...
    tone_text = {"simple": "กระชับ เน้นประเด็นสำคัญ", "academic": "ละเอียดเชิงวิชาการ", "moderate": "ชัดเจนและสมดุล"}[tone]
    content = [{"type": "input_text", "text": f"สร้างแนวคำตอบและเกณฑ์ตรวจข้อสอบเป็นภาษาไทย ระดับ {tone_text} ตามโจทย์ข้อความและภาพ ใช้ชื่อเกณฑ์และคำอธิบายที่วัดผลได้ คะแนนแต่ละเกณฑ์ไม่ติดลบและรวมกันต้องเท่ากับ {total_score} คะแนนพอดี\nโจทย์: {question_text or '(ดูภาพโจทย์)'}"}]

...
export default function CreateExam() {
  ...
  useEffect(() => {
    ...
    if (savedDraft) {
      try {
        ...
        toast.info("กู้คืนข้อมูลร่างข้อสอบเก่าให้แล้ว");
      } catch (e) {
  ...
  }, [examTitle, examDescription, startDate, startTime, endDate, endTime, isRandomized, questions, DRAFT_KEY]);

  const onBack = () => {
    if (isDirty) {
      if (window.confirm("คุณมีการกรอกข้อมูลค้างไว้ ต้องการออกจากหน้านี้ใช่หรือไม่? (ข้อมูลจะยังถูกเก็บเป็นร่างไว้อยู่)")) {
  ...
  const generateRubric = async (qId: number) => {
    ...
    const imagesToSend = q.questionImages;
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 p-4 space-y-3">
                {bankQuestions.filter(q => q.text.toLowerCase().includes(bankSearch.toLowerCase())).length === 0 ? (
                  ...
                  <div key={bq.id} className="flex items-start gap-3 p-4 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-blue-300 hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-all group">
                    ...
                    <Button
                      ...
                      className="flex-shrink-0 bg-blue-500 hover:bg-blue-600 text-white text-xs h-8 px-3 rounded-lg"
                    >
                      + เพิ่ม
                    </Button>
                  </div>
